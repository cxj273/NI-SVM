...
Mat get_frame_skip( VideoCapture cap, int skip_num )
{
    Mat origin; 
    Mat frame; 
    Mat acc; 
    ...
    frame.convertTo(acc, CV_32F);


    int avg_cnt = 1;
    for( int i = 0; i < skip_num -1; ++i )
    {
        Mat origin_skip_frame;
        Mat skip_frame;
        Mat avged_frame;

        if(!cap.read(origin_skip_frame))
        {
            break;
        }
        cvtColor(origin_skip_frame, skip_frame, CV_BGR2GRAY);

        avg_cnt++;
        skip_frame.convertTo(avged_frame, CV_32F);
        acc += avged_frame, acc;
    }

    acc /= avg_cnt;
    acc.convertTo(acc, CV_8UC1);
    cvtColor(acc, acc, CV_GRAY2BGR);
    return acc;
}
...
//	fprintf(stderr, "video size, length: %d, width: %d, height: %d\n", seqInfo.length, seqInfo.width, seqInfo.height);
	...
	while(true) {
		...
		// get a new frame
		//capture >> frame;
        //frame = get_frame_skip(capture, skip);
