...
//const int kDes = 4;
//const int kDim = 426+12;
//const int kDimFull = 436+12;
//const int kRange[4][2] = { {11, 40+3}, {41+3, 136+6}, {137+6, 244+9}, {245+9, 436+12} };
//const int kDesDim[4] = {15+3, 48+3, 54+3, 96+3};
...
const int kPCADesDim[4] = {15, 48, 54, 96};
const int kDesDim[4] = {15+3, 48+3, 54+3, 96+3};
...
int main(int argc, const char **argv)
{
    ...
    ogzstream save_file("/dev/null");
    ogzstream shot_save_file("/dev/null");
    ...
    for (int i_des = 0; i_des < kDes; ++i_des) {
        ...
        //cout << in_dim1 << ", "<< kDesDim[i_des] << endl;
        ...
        assert(in_dim1 == kPCADesDim[i_des]);
        assert(in_dim2 == kPCADesDim[i_des] * 2);
        // Debug output.
    ...
    while (getline(cin, traj_line)) {
        ...
        float raw_x  = sample_point[7];
        float raw_y  = sample_point[8];
        float raw_t  = sample_point[9];
        ...
        for (int i_des = 0; i_des < kDes; ++i_des) {
            ...
            if (i_des >= 1) {
                float sum = 0.0;
                for (int j_dim = 0; j_dim < 2*kPCADesDim[i_des]; ++j_dim)
            ...
            //float *sample_out = new float[kPCADesDim[i_des]](); // This includes the initialization to 0,

            for (int j_dim = 0; j_dim < kPCADesDim[i_des]; ++j_dim) {
                for (int k_dim = 0; k_dim < kPCADesDim[i_des]*2; ++k_dim)
                    sample_out[j_dim+3] += sub_sample[k_dim] * pca_proj[i_des][j_dim][k_dim];
            }

            // adding xyt
            //sample_out[kPCADesDim[i_des]] = raw_x;
            //sample_out[kPCADesDim[i_des]+1] = raw_y;
            //sample_out[kPCADesDim[i_des]+2] = raw_t;
...
void AddSaveClean(vector< fisher<float>* >& fishers_sum, vector< fisher<float>* >& fishers_sub, ogzstream& save_file)
{
    ...
    for (int i_des = 0; i_des < kDes; ++i_des) {
        //cout << fishers_sum[i_des]->s0 << endl;
        //cout << "i_des: " << i_des << endl;
        //fishers_sum[i_des]->show_stat();
