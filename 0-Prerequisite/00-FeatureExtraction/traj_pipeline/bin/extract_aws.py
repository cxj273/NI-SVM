...
def main(args):
    ...
    if len(args) != 3 and len(args) != 4 :
        print "extract_aws.py fps_path segx_path video_list_path [keep_shot(0|1)]"
        sys.exit(1)
    
    fps_path = args[0]
    segx_path = args[1] + "/"
    #fps_path = "./MED14_resized.all"
    #segx_path = "/home/ubuntu/Data/all_segx/"
    ...
    with open(fps_path, 'r') as f:
        for line in f:
            ...
            width = line.split()[1]
            height = line.split()[2]
            fps_num = line.split()[-1]
            fps[name] = [width, height, fps_num]
            
    
    cd = "set pipefail && cd " + os.getcwd()
    ...
    for line in file:
        ...
        record_cmd = "touch " + os.path.join(os.getcwd(), "record", record_name)
        traj_cmd = traj_bin + " " + src_path + " -i " + str(ignore_sec)
        traj_skip2_cmd = traj_cmd + " -k 2"
        traj_skip3_cmd = traj_cmd + " -k 3"
        traj_skip4_cmd = traj_cmd + " -k 4"
        traj_skip5_cmd = traj_cmd + " -k 5"
        traj_skip6_cmd = traj_cmd + " -k 6"
        ...
        ensemble_fv_xyt_cmd = "./bin/extract_fv_xyt /dev/null/" + fv_name + " ./fv_256_xyt_nosp/" + fv_name + " /dev/null/" + fv_name + " " + segx + " " + fps_num + " " + fv_xyt_256_codebook
    
        keep_shot_cmd = ''
        if len(args) == 4 and args[3] == '0':
            keep_shot_cmd = '(rm -f ./fv_nosp_noskip_shot/' + fv_name + '.gz) && '
        ...
        print "(" + cd + " && ( " + traj_cmd + " && echo Skip 1 finished! && " + traj_skip3_cmd + " && " + traj_skip6_cmd + " ) | " + ensemble_fv_cmd + " | " + ensemble_fv_xyt_cmd+ " > /dev/null && " + keep_shot_cmd + record_cmd + ") 1>/dev/null 2>/dev/null"
        #print "(" + cd + " && ( " + traj_cmd + " && echo Skip 1 finished! && " + traj_skip3_cmd + " && " + traj_skip6_cmd + " ) | " + ensemble_fv_cmd + " | " + fv_xyt_256_nosp_cmd + " > /dev/null && " + keep_shot_cmd + record_cmd + ") 1>/dev/null 2>/dev/null"
        #print "(" + cd + " && ( " + traj_cmd + " && echo Skip 1 finished! && " + traj_skip3_cmd + " && " + traj_skip6_cmd + " ) | " + ensemble_fv_cmd + " | " + fv_xyt_256_nosp_cmd + " > /dev/null && " + keep_shot_cmd + record_cmd + ") "
