...
def is_ignored(segx_path):
    with open(segx_path, 'r') as segx_file:
        lines = segx_file.readlines()
        lines[-1].rstrip('\n')
...
def main(args):
    ...
    if len(args) != 3:
        print "extract_aws.py fps_path segx_path video_list_path"
        sys.exit(1)
    ...
    for line in file:
        ...
        print "(" + cd + " && ( " + traj_cmd + " && echo Skip 1 finished! && " + traj_skip3_cmd + " && " + traj_skip6_cmd + " ) | " + ensemble_fv_cmd + " | " + fv_xyt_256_nosp_cmd + " > /dev/null && " + record_cmd + ") 1>/dev/null 2>/dev/null"
