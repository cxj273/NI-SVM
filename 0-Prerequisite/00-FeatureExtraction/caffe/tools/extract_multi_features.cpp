...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  if (argc < num_required_args) {
      LOG(ERROR) <<
          ...
          " feature_extraction_proto_file config_file_name num_mini_batches [CPU/GPU]"
          " [DEVICE_ID=0]\n";
      return 1;
  }
  ...
  */
  ...
  int num_features = 0, dim_features = 0;
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < blob_names.size(); ++i) {
      ...
      for (int n = 0; n < num_features; ++n) {
          ...
          for(int d = 0; d < dim_features; ++d)
          {
              ...
              {
                  fprintf(file_handles[i], "%d:%g ", d + 1, feature_blob_data[d]);
              }
          }
    ...
    if (batch_index % 10 == 0) {
        LOG(ERROR) << "Extracted features of " << batch_index * num_features << " query images. ";
    }
  }
  LOG(ERROR) << "Extracted features of " << num_mini_batches * num_features << " query images.";
